...
export const IssuesModule = forwardRef<IssuesModuleHandle, {
  ...
  const projectMembers = useMemo(() => {
    return users.filter((user) => user.role === "member" && conversionProject?.memberEmails.includes(user.email) && user.discipline === selected?.discipline);
  ...
  return <>
    ...
    <section className="panel issues-panel">
      <div className="panel-heading"><div><h2>Project Issues</h2><p>سجل موحد لمشاكل المشاريع، المرفقات، والتحويل إلى مهام</p></div><span className="count-badge">{filtered.length} Issues</span></div>
