...
export const IssuesModule = forwardRef<IssuesModuleHandle, {
  ...
  const [issueMentionQuery, setIssueMentionQuery] = useState<{ section: "internal" | "client"; query: string } | null>(null);
  const [issueMentionedEmails, setIssueMentionedEmails] = useState<string[]>([]);
  ...
  const mentionCandidates = useMemo(() => users.filter((user) => {
    if (!selected) return user.role !== "owner";
    return user.email !== currentUser.email && (user.role === "owner" || selected.projectCode === "" || projects.find((project) => project.code === selected.projectCode)?.memberEmails.includes(user.email));
  }), [users, currentUser.email, selected, projects]);
  const filteredIssueMentionCandidates = useMemo(() => {
    if (!issueMentionQuery) return [];
    const query = issueMentionQuery.query.toLowerCase();
    return mentionCandidates.filter((user) => `${user.displayName} ${user.email} ${user.discipline}`.toLowerCase().includes(query)).slice(0, 8);
  }, [mentionCandidates, issueMentionQuery]);
  ...
  const openIssue = useCallback((issue: Issue) => {
    ...
    setFiles([]); setClientFiles([]); setConvertEmployee(""); setConvertDueDate(today()); setConvertHours(0); setAddingCategory(false); setNewCategory(""); setIssueNoteDraft(""); setClientNoteDraft(""); setIssueMentionQuery(null); setIssueMentionedEmails([]); setEditingNoteId(null); setEditingNoteBody(""); setDrawerOpen(true);
  }, [currentUser, users, projects]);
  const openNew = useCallback(() => {
    setSelectedId(null); setForm(blankIssue(projects, currentUser, lockedProjectCode)); setFiles([]); setClientFiles([]); setConvertEmployee(""); setAddingCategory(false); setNewCategory(""); setIssueNoteDraft(""); setClientNoteDraft(""); setIssueMentionQuery(null); setIssueMentionedEmails([]); setEditingNoteId(null); setEditingNoteBody(""); setDrawerOpen(true);
  ...
  async function addIssueNote(section: "internal" | "client") {
    ...
    try {
      const activeMentions = issueMentionedEmails.filter((email) => {
        const user = users.find((item) => item.email === email);
        return user ? draft.includes(`@${user.displayName}`) : false;
      });
      const data = await jsonResponse(await fetch("/api/issue-comments", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ issueId: selected.id, section, body: draft, mentionedEmails: activeMentions }) }));
      ...
      setIssueMentionQuery(null); setIssueMentionedEmails([]);
