...
type TaskForm = Omit<Task, "id" | "createdBy" | "createdByName" | "createdByProfileImageKey" | "employeeDiscipline" | "createdAt" | "updatedAt" | "managerNote" | "submittedToManager" | "originatedByEmail" | "originatedByName" | "acceptedByEmail" | "acceptedByName" | "workCycle">;
...
export default function TaskDashboard() {
  ...
  const applyWorkspaceData = useCallback((data: WorkspaceData, initialize = false) => {
    ...
    setTaskIssueLinks(data.taskIssueLinks || []);
    setUsers(data.users || []);
  ...
  useEffect(() => {
    const onPopState = (event: PopStateEvent) => {
      ...
      const historyState = event.state as { hindazaEmployeeTasks?: unknown; hindazaEmployeeEdit?: unknown; hindazaReportTasks?: unknown; hindazaReportRow?: unknown; hindazaReportProjectEdit?: unknown; hindazaReportEmployeeEdit?: unknown } | null;
  ...
  }, [tasks, users, projects, projectDrawerReturnToReport, userDrawerReturnToReport]);
    ...
    function onKeyDown(event: KeyboardEvent) {
      if (event.key === "Escape") {
        ...
        if (projectDrawerOpen) {
          if (projectDrawerReturnToReport) {
            window.history.back();
            return;
          }
          setProjectDrawerOpen(false);
  ...
  }, [employeeTasksEmail, reportDialogMetric, projectDrawerOpen, projectDrawerReturnToReport, userDrawerOpen, userDrawerReturnToEmployeeTasks, userDrawerReturnToReport]);
  ...
  const activeWorkByProject = useMemo(() => {
    ...
    activeWork.forEach((work) => grouped.set(work.task.project, [...(grouped.get(work.task.project) || []), work]));
    return grouped;
  }, [activeWork]);
  ...
  const reportTasks = useMemo(() => customRangeInvalid ? [] : tasks.filter((task) => task.taskDate >= range.start && task.taskDate <= range.end && (
    reportScope === "all" || (reportGroup === "project" ? task.project === reportScope : task.employeeName === reportScope)
  )), [tasks, range, reportScope, reportGroup, customRangeInvalid]);
  ...
  }, [reportTasks, reportGroup, projectCodes, employees, projects]);
  ...
  function exportPdf() {
    ...
    const bars = displayedReportRows.map((row) => {
      ...
      return `<div class="bar-row"><strong>${escapeXml(row.label)}${reportGroup === "employee" ? ` (${row.projectCount} projects)` : ""}</strong><div class="bar"><i class="approved" style="width:${scale(row.approved)}%">${row.approved || ""}</i><i class="wip" style="width:${scale(row.wip)}%">${row.wip || ""}</i><i class="pending" style="width:${scale(row.pending)}%">${row.pending || ""}</i><i class="returned" style="width:${scale(row.returned)}%">${row.returned || ""}</i></div><span>${row.total}</span></div>`;
    }).join("");
    popup.document.write(`<!doctype html><html dir="ltr"><head><meta charset="utf-8"><title>HINDAZA Project Management Report</title><style>*{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact}body{font-family:Arial,Tahoma,sans-serif;color:#1d1d1d;padding:26px;text-align:left}.logo{display:block;width:220px;max-height:76px;object-fit:contain;object-position:left center;margin:0 0 20px}h1{color:#171717;margin:0;border-left:5px solid #ffd200;padding-left:12px}.meta{color:#737373;margin:8px 0 22px}.summary{display:grid;grid-template-columns:repeat(4,minmax(110px,1fr));gap:10px;margin:20px 0}.summary div{border:1px solid #d9d9d2;border-radius:10px;padding:11px 14px}.summary strong{font-size:22px;color:#8b6c00;display:block;margin-top:4px}.chart{margin:22px 0;padding:14px;border:1px solid #deded8;border-radius:10px}.bar-row{display:grid;grid-template-columns:190px 1fr 35px;gap:10px;align-items:center;margin:9px 0;font-size:11px}.bar{display:flex;height:18px;background:#f1f1ed;border:1px solid #bfc3c3;border-radius:8px;overflow:hidden}.bar i{display:grid;height:100%;min-width:0;place-items:center;color:white;font-size:9px;font-style:normal;text-shadow:0 1px 2px rgba(0,0,0,.45)}.approved{background:#2f8a64!important}.wip{background:#627b87!important}.pending{background:#ffd200!important;color:#171717!important;text-shadow:none!important}.returned{background:#d05a62!important}.legend{display:flex;flex-wrap:wrap;gap:16px;margin-top:13px;font-size:10px}.legend span{display:inline-flex;align-items:center;gap:6px}.legend i{width:18px;height:8px;border:1px solid #999;border-radius:4px}.footer{margin-top:30px;color:#858585;font-size:9px}@media print{body{padding:0}}</style></head><body><img class="logo" src="/report-logo.png" alt="HINDAZA"><h1>HINDAZA Project Management</h1><div class="meta">${reportPeriodLabel} Report · ${formatDate(range.start)} — ${formatDate(range.end)}</div><div class="summary"><div>Total Tasks<strong>${reportSummary.total}</strong></div><div>Manager Approved<strong>${reportSummary.approved}</strong></div><div>WIP<strong>${reportSummary.wip}</strong></div><div>Needs Review<strong>${reportSummary.pending}</strong></div></div><div class="chart">${bars}<div class="legend"><span><i class="approved"></i>Manager Approved</span><span><i class="wip"></i>WIP</span><span><i class="pending"></i>Needs Review</span><span><i class="returned"></i>Returned</span></div></div><div class="footer">Generated from HINDAZA Project Management</div><script>window.onload=()=>{window.print();}</script></body></html>`);
      ...
      <main className="main-content">
        ...
        {tab === "projects" && selectedProject && <section className="project-workspace" aria-label={`${selectedProject.name} workspace`}>
          ...
          {projectWorkspaceTab === "tasks" && <>
            ...
            <TaskTable loading={loading} tasks={projectTasks} filteredCount={projectTasks.length} tab={tab} employees={employeeOptions} users={users} projects={[selectedProject.code]} lockedProjectCode={selectedProject.code} search={search} employeeFilter={employeeFilter} projectFilter={projectFilter} statusFilter={statusFilter} reviewFilter={reviewFilter} dueDateFilter={dueDateFilter} disciplineFilter={disciplineFilter} showEmployeeFilter={currentUser?.role !== "member" || currentUserIsProjectManager} showDisciplineColumn={currentUser?.role === "owner" || currentUser?.role === "manager" || currentUserIsProjectManager} setSearch={setSearch} setEmployeeFilter={setEmployeeFilter} setProjectFilter={setProjectFilter} setStatusFilter={setStatusFilter} setReviewFilter={setReviewFilter} setDueDateFilter={setDueDateFilter} setDisciplineFilter={setDisciplineFilter} openTask={openTask} showAll={() => undefined} timeEntries={timeEntries} clock={clock} commentCounts={taskCommentCounts} subtasks={subtasks} attachments={taskAttachments} issueLinks={taskIssueLinks} openIssue={openLinkedIssue} />
          </>}
        ...
        {tab === "tasks" && <>
          ...
          <TaskTable loading={loading} tasks={filteredTasks} filteredCount={filteredTasks.length} tab={tab} employees={employeeOptions} users={users} projects={projectCodes} search={search} employeeFilter={employeeFilter} projectFilter={projectFilter} statusFilter={statusFilter} reviewFilter={reviewFilter} dueDateFilter={dueDateFilter} disciplineFilter={disciplineFilter} showEmployeeFilter={currentUser?.role !== "member"} showDisciplineColumn={currentUser?.role === "owner" || currentUser?.role === "manager"} setSearch={setSearch} setEmployeeFilter={setEmployeeFilter} setProjectFilter={setProjectFilter} setStatusFilter={setStatusFilter} setReviewFilter={setReviewFilter} setDueDateFilter={setDueDateFilter} setDisciplineFilter={setDisciplineFilter} openTask={openTask} showAll={() => setTab("tasks")} timeEntries={timeEntries} clock={clock} commentCounts={taskCommentCounts} subtasks={subtasks} attachments={taskAttachments} issueLinks={taskIssueLinks} openIssue={openLinkedIssue} />
        </>}
...
function ReportTasksDialog({ metric, title, groupBy, tasks, projects, users, timeEntries, clock, projectCode = "", employee = null, onOpenProject, onProjectSettings, onEmployeeSettings, onClose, onOpenTask }: { metric: ReportMetric; title?: string; groupBy: "project" | "employee"; tasks: Task[]; projects: Project[]; users: User[]; timeEntries: TaskTimeEntry[]; clock: number; projectCode?: string; employee?: User | null; onOpenProject?: (projectCode: string) => void; onProjectSettings?: (project: Project) => void; onEmployeeSettings?: (user: User) => void; onClose: () => void; onOpenTask: (task: Task) => void; }) {
  ...
  const printReportTasks = () => {
    ...
    const printedGroups = groups.map(([key, groupTasks]) => {
      ...
      const rows = groupTasks.map((task) => {
        const logged = taskLoggedHours(task, timeEntries.filter((entry) => entry.taskId === task.id), clock);
  ...
  return <div className="drawer-layer employee-tasks-layer report-tasks-layer" role="dialog" aria-modal="true" aria-label={`${dialogTitle} report tasks`}>
    ...
    <section className="employee-tasks-dialog report-tasks-dialog">
      ...
      <div className={`employee-project-groups report-task-group-count-${Math.min(groups.length, 3)}`} ref={groupsRef} onWheel={handleWheel}><div className="employee-project-groups-content">{groups.length === 0 ? <div className="comments-empty">No tasks match this report filter.</div> : groups.map(([key, groupTasks]) => {
        ...
        return <section className="employee-project-group" key={key}><button type="button" className={`employee-project-heading${employee && project && onProjectSettings ? " with-settings" : ""}`} onClick={() => toggleGroup(key)} aria-expanded={!collapsed}><span>{collapsed ? "+" : "−"}</span><div><strong>{groupBy === "project" ? project?.name || key : groupEmployee?.displayName || key}</strong><small>{groupBy === "project" ? key : groupEmployee?.discipline || "Employee"}</small></div><em>{groupTasks.length}</em>{employee && project && onProjectSettings && <span className="report-project-settings" role="button" tabIndex={0} title={`Project settings for ${project.name}`} aria-label={`Project settings for ${project.name}`} onClick={(event) => { event.stopPropagation(); onProjectSettings(project); }} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); event.stopPropagation(); onProjectSettings(project); } }}>⚙</span>}</button>{!collapsed && <div className={`employee-task-table-wrap${groupTasks.length > 4 ? " has-more-tasks" : ""}`} tabIndex={groupTasks.length > 4 ? 0 : undefined}><table className="employee-task-table"><thead><tr><th>Task</th><th>Employee</th><th>Created By</th><th>Created Date</th><th>Due Date</th><th>Status</th><th>Manager Review</th><th>Hours</th><th>Indicator</th></tr></thead><tbody>{groupTasks.map((task) => {
          ...
          return <tr className="employee-task-clickable-row" key={task.id} onClick={(event) => { if (event.ctrlKey || event.metaKey || event.shiftKey || event.button !== 0 || (event.target as HTMLElement).closest("a, button")) return; onOpenTask(task); }}><td><a href={href} onClick={openLink}><strong dir="auto">{task.title}</strong><small>{task.expectedOutput || "—"}</small></a></td><td><a href={href} onClick={openLink}>{task.employeeName || "Unassigned"}</a></td><td><a href={href} onClick={openLink}>{task.createdByName || "Unknown user"}</a></td><td dir="ltr"><a href={href} onClick={openLink}>{formatCreatedDate(task.createdAt)}</a></td><td dir="ltr"><a href={href} onClick={openLink}>{formatDueDate(task.taskDate)}</a></td><td><a href={href} onClick={openLink}><span className={`pill status-${task.status}`}>{tableStatusLabel[task.status]}</span></a></td><td><a href={href} onClick={openLink}><span className={`pill check-${task.managerCheck}`}>{tableCheckLabel[task.managerCheck]}</span></a></td><td><a href={href} onClick={openLink}>{logged ? `${logged.toFixed(2)}h` : "—"}</a></td><td><a href={href} onClick={openLink}><span className={`flag flag-${flag.key}`}>{flag.label}</span></a></td></tr>;
...
function EmployeeTasksDialog({ employee, tasks, projects, timeEntries, clock, onClose, onOpenTask, onEditEmployee }: { employee: User | null; tasks: Task[]; projects: Project[]; timeEntries: TaskTimeEntry[]; clock: number; onClose: () => void; onOpenTask: (task: Task) => void; onEditEmployee: (user: User) => void; }) {
    ...
    <section className="employee-tasks-dialog">
      ...
      <div className="employee-project-groups" ref={projectGroupsRef} onWheel={handleDialogWheel}><div className="employee-project-groups-content">{groups.length === 0 ? <div className="comments-empty">No tasks match this filter · لا توجد مهام مطابقة</div> : groups.map(([projectCode, projectTasks]) => {
        ...
        return <section className="employee-project-group" key={projectCode}><button type="button" className="employee-project-heading" onClick={() => toggleProject(projectCode)} aria-expanded={!collapsed}><span>{collapsed ? "+" : "−"}</span><div><strong>{project?.name || projectCode}</strong><small>{projectCode}</small></div><em>{projectTasks.length}</em></button>{!collapsed && <div className={`employee-task-table-wrap${projectTasks.length > 4 ? " has-more-tasks" : ""}`} tabIndex={projectTasks.length > 4 ? 0 : undefined} aria-label={projectTasks.length > 4 ? `${project?.name || projectCode}: scroll for more tasks` : undefined}><table className="employee-task-table"><thead><tr><th>Task</th><th>Created By</th><th>Created Date</th><th>Due Date</th><th>Status</th><th>Manager Review</th><th>Hours</th><th>Indicator</th></tr></thead><tbody>{projectTasks.map((task) => {
          ...
          const active = task.status === "in_progress" || taskEntries.some((entry) => !entry.endedAt);
          const flag = taskFlag(task);
          const href = `/?view=projects&project=${encodeURIComponent(task.project)}&section=tasks&task=${task.id}`;
          ...
          return <tr className="employee-task-clickable-row" key={task.id} onClick={(event) => { if (event.ctrlKey || event.metaKey || event.shiftKey || event.button !== 0 || (event.target as HTMLElement).closest("a, button")) return; onOpenTask(task); }}><td><a href={href} onClick={openTaskLink}><span className="employee-task-title"><strong dir="auto">{task.title}</strong>{active && <i className="employee-task-live-pulse" title="Working now" aria-label="Working now" />}</span><small>{task.expectedOutput || "—"}</small></a></td><td><a href={href} onClick={openTaskLink}>{task.createdByName || "Unknown user"}</a></td><td dir="ltr"><a href={href} onClick={openTaskLink}>{formatCreatedDate(task.createdAt)}</a></td><td dir="ltr"><a href={href} onClick={openTaskLink}>{formatDueDate(task.taskDate)}</a></td><td><a href={href} onClick={openTaskLink}><span className={`pill status-${task.status}`}>{tableStatusLabel[task.status]}</span></a></td><td><a href={href} onClick={openTaskLink}><span className={`pill check-${task.managerCheck}`}>{tableCheckLabel[task.managerCheck]}</span></a></td><td><a href={href} onClick={openTaskLink}>{logged ? `${logged.toFixed(2)}h` : "—"}</a></td><td><a href={href} onClick={openTaskLink}><span className={`flag flag-${flag.key}`}>{flag.label}</span></a></td></tr>;
        })}</tbody></table></div>}</section>;
      })}</div></div>
    </section>
  </div>;
...
function TaskTable(props: TaskTableProps) {
  ...
  return <section className="panel">
    ...
    {props.loading ? <div className="loading-state"><div className="spinner" /><p>جاري تحميل المهام...</p></div> : props.tasks.length === 0 ? <div className="empty-state"><strong>لا توجد مهام مطابقة</strong><p>غيّر خيارات البحث أو أضف مهمة جديدة.</p></div> : <><div className="task-table-wrap"><table className={`task-table task-data-table task-table-ltr${props.showEmployeeFilter ? "" : " member-task-table"}`}><thead><tr><th>Task</th>{props.showEmployeeFilter && <th>Employee</th>}<th>Created By</th>{props.showDisciplineColumn && <th>Discipline</th>}<th>Created Date</th><th>Due Date</th><th>Priority</th><th>Hours</th><th>Status</th><th>Manager Review</th><th>Indicator</th><th>Issue Link</th></tr></thead><tbody>{props.tasks.map((task) => {
      const flag = taskFlag(task);
      const entries = props.timeEntries.filter((entry) => entry.taskId === task.id);
      const taskSubtasks = props.subtasks.filter((subtask) => subtask.taskId === task.id);
      const attachmentCount = props.attachments.filter((attachment) => attachment.taskId === task.id).length;
      const issueLink = props.issueLinks.find((link) => link.convertedTaskId === task.id);
...
function UserDrawer({ selectedEmail, selectedUser, form, setForm, setOpen, saveUser, deleteUser, saving, currentUser, projects, onProfileImageChange }: { selectedEmail: string | null; selectedUser: User | null; form: UserForm; setForm: (value: UserForm) => void; setOpen: (value: boolean) => void; saveUser: (event: FormEvent) => void; deleteUser: () => void; saving: boolean; currentUser: User | null; projects: Project[]; onProfileImageChange: (email: string, profileImageKey: string) => void; }) {
