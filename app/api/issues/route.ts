...
async function validRaisedBy(db: Awaited<ReturnType<typeof getDb>>, email: string, discipline: (typeof disciplines)[number], projectCode: string) {
  ...
  const [raisedBy] = await db.select({ email: users.email, displayName: users.displayName, discipline: users.discipline }).from(users)
...
async function issueRows() {
  ...
  return issues.map((issue) => ({ ...issue, attachments: byIssue.get(issue.id) || [] }));
}
...
export async function POST(request: Request) {
  try {
    ...
    const discipline = currentUser.role === "member"
      ? enumValue(currentUser.discipline, disciplines, requestedDiscipline)
      : requestedDiscipline;
    if (currentUser.role === "member" && currentUser.discipline !== discipline) {
      return Response.json({ error: "Employees can create issues only in their discipline." }, { status: 403 });
    }
    ...
    return Response.json({ issue: { ...inserted[0], attachments: [] } }, { status: 201 });
  } catch (error) {
    const unauthorized = unauthorizedResponse(error);
    if (unauthorized) return unauthorized;
